...
const OrderItem = ({id, key, sum, date, description, orderedGoods, selectOrder, orderSelected, status}) => {
        ...
        return (
            <div key={key} className='flex-column items-center ba ma2 bg-lightest-blue hover-dark-blue'>
                <div id={id} className='flex items-center w-100 f5 underline-hover justify-between ma1 pointer'
                    ...
                    <p className='w-10 ma0 pa2 tc'>{date}</p>
                    <p className='w-10 ma0 pa2 tc'>{status}</p>
                    <p className='w-60 ma0 pa2 tc'>{description.replace('%товаров%', orderedGoods.length)}</p>
                    <p className='w-10 ma0 pa2 tc'>{`${Number(sum).toFixed(2)}`}</p>
                </div>
